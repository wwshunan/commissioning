...
from .phasescan.phase_fit import PhaseFit
...
@bp.route('/commissioning/phasescan/config')
def config():
    fname = os.path.join(basedir, 'phasescan/resources/config.txt')
...
@bp.route('/commissioning/phasescan/phase-set', methods=['POST'])
def phase_set():
    data = request.get_json()
    rf_write_pv = data['cavity_write_pv']
    rf_phase_val = data['cavity_phase_val']
    cavity_response_time = data['cavity_response_time']
    bpm_read_num = data['bpm_read_num']
    bpm_read_sep = data['bpm_read_sep']

    if data['scan_model'] == 'single':
        bpm_pv_name = data['bpm_phase_pv']
        phase_pv = PV(rf_write_pv)
        phase_pv.put(rf_phase_val)
        time.sleep(cavity_response_time)
        bpm_vals = []
        bpm_pv = PV(bpm_pv_name)
        for i in range(bpm_read_num):
            bpm_val = bpm_pv.get()
            bpm_vals.append(bpm_val)
            time.sleep(bpm_read_sep)
        bpm_val = np.average(bpm_vals)
        bpm_std = np.std(bpm_vals)
    return jsonify({
        'point': {'x': rf_phase_val, 'y': bpm_val},
        'bpm_error': bpm_std
    })


@bp.route('/commissioning/phasescan/curve-fit', methods=['POST'])
async def curve_fit():
    #data = request.get_json()
    #cavity_name = data['cavity_name']
    #synch_phase_fname = os.path.join(basedir, 'phasescan/resources/synch_phases.txt')
    #df = pd.read_csv(synch_phase_fname, header=0, sep=r'\s+')
    #synch_phase = df[df['cavity_name'] == cavity_name]['synch_phase'].values[0]

    #config_fname = os.path.join(basedir, 'phasescan/resources/config.txt')
    #config_infos = pd.read_csv(config_fname, header=0, sep='\s+')
    #cavity_info = config_infos[config_infos['cavity_name'] == cavity_name]

    #payload = {'distance': cavity_info['distance'].values[0],
    #           'field_name': cavity_info['field_name'].values[0] + '.txt',
    #           'sync_phase': synch_phase,
    #           'freq': 162.5e6,
    #           'bpm_harm': 1,
    #           'bpm_polarity': 1,
    #           'Epk_ref': cavity_info['Epk_ref'].values[0],
    #           'rf_direction': cavity_info['rf_direction'].values[0]
    #           }

    #payload = {**payload, **data}
    #fit_obj = PhaseFit(payload)
    #result = await fit_obj.fit()
