...
class ADSTask(AbstractTaskImpl, CallableTask):
    ...
    def execUserCode(self, executor):
        task = {}
        task['id'] = self.id
        task['task'] = executor.enqueue(self.userCode.execUserCode)
        return task
...
class ADSSequence(AbstractTaskImpl, Sequence):
    def __init__(self, parameter=None, deviceName='', sequenceName='',
                 sequence=None, reference=None, taskName='', repititionNumber=1,
                 repititionCount=0, description='', skippable=False, interactive=False,
                 taskBreakPoint=False, skipTaskState=False, skippableState=False, id=1, parallelizable=False):
        self.id = id
        self.parameter = parameter
        self.deviceName = deviceName
        self.sequenceName = sequenceName
    ...
    def execUserCode(self, executor):
        ...
        for task in self.getFlattenedTaskList():
            item = {}
            item['id'] = task.id
            item['task'] = executor.enqueue(task.userCode.execUserCode)
            tasks.append(item)
        return tasks
